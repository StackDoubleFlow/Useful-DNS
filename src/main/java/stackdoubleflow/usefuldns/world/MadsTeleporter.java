...
public class MadsTeleporter extends Teleporter {
	...
	private final Long2ObjectMap<MadsTeleporter.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(
			4096);

	public MadsTeleporter(WorldServer worldIn, int dme2, double x, double y, double z) {
	...
	public boolean makePortal(Entity entityIn) {
				...
				for (int l7 = 1; l7 < 3; ++l7) {
					for (int k8 = -1; k8 < 3; ++k8) {
						...
						this.worldServerInstance.setBlockState(new BlockPos(k9, k10, k11),
								flag ? Blocks.STONE.getDefaultState() : Blocks.AIR.getDefaultState());
		...
		IBlockState iblockstate = BlockInit.MADS_PORTAL.getDefaultState().withProperty(BlockPortal.AXIS,
			...
			for (int l8 = 0; l8 < 4; ++l8) {
				for (int l9 = -1; l9 < 4; ++l9) {
					...
					this.worldServerInstance.setBlockState(new BlockPos(l10, l11, k12),
							flag1 ? BlockInit.DECANYSODIUM.getDefaultState() : iblockstate, 2);
