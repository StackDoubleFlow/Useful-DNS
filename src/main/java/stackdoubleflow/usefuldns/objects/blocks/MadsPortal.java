...
public class MadsPortal extends BlockPortal implements IHasModel {
	...
	public boolean trySpawnPortal(World worldIn, BlockPos pos) {
		MadsPortal.Size blockportal$size = new MadsPortal.Size(worldIn, pos, EnumFacing.Axis.X);

		if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
			blockportal$size.placePortalBlocks();
			return true;
		} else {
			MadsPortal.Size blockportal$size1 = new MadsPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
	...
	public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
		...
		MadsPortal.Size blockportal$size = new MadsPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
		LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

		if (!blockportal$size.isValid()) {
			enumfacing$axis = EnumFacing.Axis.X;
			blockportal$size = new MadsPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
		}
	...
	public static class Size {
		...
		protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
			...
			for (i = 0; i < 22; ++i) {
				...
				if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
						|| this.world.getBlockState(blockpos.down()).getBlock() != BlockInit.DECANYSODIUM) {
					break;
				}
			}

			Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
			return block == BlockInit.DECANYSODIUM ? i : 0;
		}
		...
		protected int calculatePortalHeight() {
			...
			for (this.height = 0; this.height < 21; ++this.height) {
				for (int i = 0; i < this.width; ++i) {
					...
					if (block == BlockInit.MADS_PORTAL) {
					...
					if (i == 0) {
						...
						if (block != BlockInit.DECANYSODIUM) {
							break label56;
						}
			...
			for (int j = 0; j < this.width; ++j) {
				if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height))
						.getBlock() != BlockInit.DECANYSODIUM) {
					this.height = 0;
					break;
				}
			}
		...
		protected boolean isEmptyBlock(Block blockIn) {
			return blockIn.getMaterial(blockIn.getBlockState().getBaseState()) == Material.AIR || blockIn == Blocks.FIRE || blockIn == BlockInit.MADS_PORTAL;
		}
			...
			for (int i = 0; i < this.width; ++i) {
				...
				for (int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j),
							BlockInit.MADS_PORTAL.getDefaultState().withProperty(MadsPortal.AXIS, this.axis), 2);
				}
			}
		}
	}
